import FadSDK from '../sdk/fad-sdk.min.js';
import {
  REGULA_CREDENTIALS,
  CONFIGURATION,
  TOKEN,
} from './regula-constants.js';
...
async function initProcess() {
  ...
  try {
    ...
    if (ex.code === FadSDK.Errors.Regula.CAMERA_PERMISSION_DENIED) {
      ...
      // restart component
      alert(
        'Foto del rostro de la identificación no encontrada, favor de reintentar el proceso',
      );
    } else if (ex.code === FadSDK.Errors.Regula.OCR_NOT_FOUND) {
      // restart component
      alert(
        'OCR de la identificación no encontradO, favor de reintentar el proceso',
      );
    } else {
      // restart component
